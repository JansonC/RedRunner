...
namespace RedRunner.Utilities
{
    ...
    {
        [SerializeField] protected PathDefinition m_PathDefinition;
        [SerializeField] protected float m_Step = 0.1f;
        [SerializeField] protected bool m_FollowSpeeds = true;
        [SerializeField] protected float m_Speed = 1f;
        [SerializeField] protected bool m_FollowDelays = true;
        [SerializeField] protected float m_Delay = 0f;
        [SerializeField] protected bool m_Smart = false;
        [SerializeField] protected Vector3 m_RangeSize = new Vector3(4f, 4f, 4f);
        [SerializeField] protected Vector3 m_RangeOffset = new Vector3(0f, 0f, 0f);
        ...
        public virtual Vector3 Velocity => m_Velocity;
        ...
        void Update()
        {
            if (m_Smart)
            {
                Collider2D[] colliders = Physics2D.OverlapBoxAll(transform.position + m_RangeOffset, m_RangeSize, 0f,
            ...
            {
                transform.position =
                    Vector3.MoveTowards(transform.position, m_CurrentPoint.Current.transform.position, speed);
            }
            ...
            else if (m_CurrentPoint.Current.moveType == PathPoint.MoveType.Acceleration)
            {
                ...
                transform.position = Vector3.MoveTowards(transform.position, m_CurrentPoint.Current.transform.position,
                    m_OverTimeSpeed);
