...
namespace RedRunner.TerrainGeneration
{
    ...
    {
        public static TerrainGenerator Singleton { get; private set; }
        ...
        [SerializeField] protected TerrainGenerationSettings m_Settings;
        ...
        [SerializeField] protected float m_DestroyRange = 100f;
        [SerializeField] protected float m_GenerateRange = 100f;
        [SerializeField] protected float m_BackgroundGenerateRange = 200f;
        ...
        public float PreviousX => m_PreviousX;

        public float CurrentX => m_CurrentX;

        public TerrainGenerationSettings Settings => m_Settings;
        ...
        public virtual void Generate()
        {
            ...
            for (int i = 0; i < m_BackgroundLayers.Length; i++)
            {
                ...
                if (block != null && (m_BackgroundLayers[i].LastBlock == null ||
                                      newX < m_Character.transform.position.x + m_BackgroundGenerateRange))
                {
        ...
        public virtual void Remove()
        {
            ...
            foreach (Block t in blocksToRemove)
            {
                RemoveBlock(t);
            }

            foreach (BackgroundBlock t in backgroundBlocksToRemove)
            {
                RemoveBackgroundBlock(t);
            }
        }
        ...
        public virtual bool CreateBackgroundBlock(BackgroundBlock blockPrefab, Vector3 position, BackgroundLayer layer,
            int layerIndex)
        {
            if (blockPrefab == null)
            {
        ...
        public Block GetCharacterBlock()
        {
            ...
            foreach (KeyValuePair<Vector3, Block> block in m_Blocks)
            {
                if (block.Key.x <= m_Character.transform.position.x &&
                    block.Key.x + block.Value.Width > m_Character.transform.position.x)
                {
        ...
        public static Block ChooseFrom(Block[] blocks)
        {
            ...
            foreach (Block t in blocks)
            {
                if (randomPoint < t.Probability)
                {
                    return t;
                }
                else
                {
                    randomPoint -= t.Probability;
                }
            }
