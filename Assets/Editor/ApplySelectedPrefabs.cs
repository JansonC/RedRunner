...
public class ApplySelectedPrefabs : EditorWindow
{
    public delegate void ApplyOrRevert(GameObject _goCurrentGo, Object _ObjPrefabParent,
        ReplacePrefabOptions _eReplaceOptions);
    ...
    static void SearchPrefabConnections(ApplyOrRevert _applyOrRevert)
    {
        ...
        if (tSelection.Length > 0)
        {
            ...
            foreach (GameObject go in tSelection)
            {
                ...
                {
                    //Prefab Root;
                    goPrefabRoot = PrefabUtility.GetCorrespondingObjectFromSource(go).transform.root
                        .gameObject;
                    goCur = go;
                    ...
                    while (goCur.transform.parent != null && !bTopHierarchyFound)
                    {
                        ...
                        if (goPrefabRoot ==
                            PrefabUtility.GetCorrespondingObjectFromSource(goCur.transform.parent
                        ...
                        else
                        {
                            ...
                            if (goPrefabRoot != PrefabUtility.GetCorrespondingObjectFromSource(goCur))
                            {
